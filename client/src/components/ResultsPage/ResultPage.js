...
class ResultPage extends React.Component {
  ...
  getFlights = e => {
    ...
    $.post("/api/midpoint", data).then(midpointRes => {
      const destITACode = midpointRes.data[0][0].ita;
      this.setState({
        destITACode: destITACode,
        destAirCoordinates: [
          midpointRes.data[0][0].lattitude,
          midpointRes.data[0][0].longitude
        ],
      ...
      $.get(
        `http://developer.goibibo.com/api/search/?app_id=ad6a1a69&app_key=dcf3fe52cb4920b668f623315303b99f&format=json&source=${
          this.state.airport
        }&destination=${destITACode}&dateofdeparture=${date1[0]}${date1[1]}${
          date1[2]
        ...
        let directFlights = {};
        let cheapestFare =
          userFlights.data.data.onwardflights[0].fare.adulttotalfare;
        for (let i = 0; i < userFlights.data.data.onwardflights.length; i++) {
          if (
            userFlights.data.data.onwardflights[i].destination ===
              destITACode &&
            userFlights.data.data.onwardflights[i].fare.adulttotalfare <
              cheapestFare
          ) {
            cheapestFare =
              userFlights.data.data.onwardflights[i].fare.adulttotalfare;
      ...
      $.get(
        ...
        for (let i = 0; i < res.data.data.onwardflights.length; i++) {
          if (
            res.data.data.onwardflights[i].destination === destITACode &&
  ...
  getMidPoint = () => {
    $.get(`/airports/${localStorage.clsr_id}`)
      ...
      .then(resp => {
        console.log(this.state.params22);
